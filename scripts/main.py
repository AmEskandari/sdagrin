...
def has_graph_support(model_cls):
    return model_cls in [models.SDAGRIN]


def get_model_classes(model_str, window=36,num_heads=1,hidden_units=64):
    if model_str == 'sdagrin':
        model, filler = models.SDAGRIN, fillers.GraphFiller
    else:
        raise ValueError(f'Model {model_str} not available.')
    return model, filler


def get_dataset(dataset_name,pnoise):
    if dataset_name[:3] == 'air':
        ...
        dataset = datasets.MissingValuesMetrLA(p_fault=0., p_noise=pnoise)
    elif dataset_name == 'bay_point':
        dataset = datasets.MissingValuesPemsBay(p_fault=0., p_noise=pnoise)
    else:
...
def parse_args():
    ...
    parser.add_argument("--model-name", type=str, default='sdagrin')
    ...
    parser.add_argument('--pnoise', type=float, default=0.25)
...
def run_experiment(args):
    ...
    dataset = get_dataset(args.dataset_name,args.pnoise)
   ...
   ########################################
    ...
    ########################################
    if args.evaluation:
        filler.load_state_dict(torch.load(args.checkpoint_dir,
