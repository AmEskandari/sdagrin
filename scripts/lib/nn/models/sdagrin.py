...
class SDAGRIN(nn.Module):
    ...
    def forward(self, x, mask=None, u=None, **kwargs):
        ...
        z = torch.where(mask, x, torch.zeros(mask.shape).to(x.device))
        
        # Multi-Head Attention part
        h = rearrange(z.squeeze(dim=3), 'a b c -> c a b')
        k = self.key(h)
        q = self.query(h)
        v = self.value(h)
        _, att_adj = self.att(q, k, v)
        att_adj = torch.mean(att_adj, dim=0)
        A = torch.where(adj > 0, att_adj, torch.zeros_like(att_adj))
